...
```{r}
table(is.na(hiv$bsi_depress)) |> prop.table()

# Create an index of row numbers containing missing values 
...
```{r, echo= false}
imp1.data <- data.frame(value = bsi_depress.ums)
imp1.data$imputed <- "FALSE"
imp1.data$imputed[miss.dep.idx] <- "TRUE"

ggboxplot(imp1.data, y="value", fill = "imputed", x = "imputed")
```
...
```{r, echo= false}
imp2.data <- data.frame(value = bsi_depress.hotdeck)
imp2.data$imputed <- "FALSE"
imp2.data$imputed[miss.dep.idx] <- "TRUE"
...
And using predictive mean matching. 
```{r}
pmm.impute <- regressionImp(bsi_depress ~ gender + siblings + age, hiv)
```

```{r, echo= false}
imp3.data <- data.frame(value = bsi_depress.lm)
imp3.data$imputed <- FALSE
imp3.data$imputed[miss.dep.idx] <- TRUE
imp3.data$method <- "lm"

imp.reg <- pmm.impute %>% 
  select(value = bsi_depress, 
         imputed = bsi_depress_imp) %>%
  mutate(method = "pmm") %>%
  bind_rows(imp3.data)

ggviolin(imp.reg, y="value", 
         x = "method", fill="imputed",
         add = "boxplot", 
         alpha = .2)
```
...
```{r, echo= false}
impe.data <- data.frame(value = bsi_depress.lm.resid)
impe.data$imputed <- "FALSE"
impe.data$imputed[miss.dep.idx] <- "TRUE"
...
```{r}
ggboxplot(iris_long, y="ratio", x="Species", facet.by = ".imp")
```
